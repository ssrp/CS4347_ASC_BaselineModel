    ...
    def forward(self, x_audio, x_spectrum, x_features, x_fmstd):
        ...
        for b in range(self.dn_parameters['audio']['nb_blocks']):
            ...
            for l in range(nb_layers):
                ...
                i_denseBlock += 1
                x_audio = torch.cat((x_audio, previous_state_audio), dim=2)
        ...
        for b in range(self.dn_parameters['spectrum']['nb_blocks']):
            ...
            for l in range(nb_layers):
                ...
                i_denseBlock += 1
                x_spectrum = torch.cat((x_spectrum, previous_state_spectrum), dim=3)
        ...
        x_spectrum = self.nn_spectrum_lastLayer[0](x_spectrum)    # Fully Connected
        x_spectrum = F.relu(x_spectrum)
        x_spectrum = self.nn_spectrum_lastLayer[0](x_spectrum)    # Dropout
        ...
        for b in range(self.dn_parameters['features']['nb_blocks']):
            ...
            for l in range(nb_layers):
                ...
                i_denseBlock += 1
                x_features = torch.cat((x_features, previous_state_features), dim=2)
            ...
            if b != self.dn_parameters['features']['nb_blocks'] - 1:
                ...
                i_denseTransitionBlock += 1
                x_features = F.relu(x_features)
                x_features = self.nn_features_denseTransitinoBlock[i_denseTransitionBlock](x_features)  # Convolution
                i_denseTransitionBlock += 1
                x_features = self.nn_features_denseTransitinoBlock[i_denseTransitionBlock](x_features)  # Dropout
                i_denseTransitionBlock += 1
                x_features = self.nn_features_denseTransitinoBlock[i_denseTransitionBlock](x_features)  # Max_features Pooling
                i_denseTransitionBlock += 1
